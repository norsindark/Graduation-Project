...
const AddressEdit = ({ currentAddress, onEditSuccess, setShowAddressEdit }: {
    currentAddress: {
        id: string, street: string, city: string, state: string, country: string,
        postalCode: number, phoneNumber: string, addressType: string, userId: string
    }, onEditSuccess: () => void, setShowAddressEdit: (show: boolean) => void
}) => {
    ...
    const onFinish = async (
        values: {
            ...
            postalCode: number, phoneNumber: string, addressType: string
        }) => {
        const { street, city, state, country, postalCode, phoneNumber, addressType } = values;
        if (!userId) {
            notification.error({
        ...
        try {
            ...
            notification.error({
                ...
                description: error instanceof Error ? error.message : "Error during registration process!",
                duration: 5,
                showProgress: true
            })
        } finally {
    ...
    return (
        <div className="container text-medium">
            <h4 className="fp__dsahboard_overview_item text-center flex justify-center items-center p-3 font-[500] text-[18px]">Edit Address</h4>
            <Form
                form={form}
                layout="vertical"
            ...
            >
                ...
                <Button danger size="large" style={{ fontWeight: 'medium', margin: '0 10px' }} shape="round" type="primary" className="cancel_new_address" onClick={() => setShowAddressEdit(false)}>
